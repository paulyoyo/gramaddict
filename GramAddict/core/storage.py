...
class InteractionStore(Protocol):
    """What the bot needs from account storage. Storage (JSON files) is the real
    one; tests use an in-memory fake (tests/fakes.py)."""

    account_path: str
    filter_path: str

    def is_user_in_blacklist(self, username) -> bool: ...
    def is_user_in_whitelist(self, username) -> bool: ...
    def check_user_was_interacted(self, username): ...
    def can_be_reinteract(self, last_interaction, hours_that_have_to_pass) -> bool: ...
    def can_be_unfollowed(self, last_interaction, days_that_have_to_pass) -> bool: ...
    def get_following_status(self, username): ...
    def get_unfollowable_users(self, unfollow_delay_days, count): ...
    def add_interacted_user(self, username, session_id, **fields) -> None: ...
    def add_filter_user(self, username, profile_data, skip_reason=None) -> None: ...
    def mark_ella_target(self, username) -> None: ...
    def get_source_position(self, source, job_type) -> int: ...
    def save_source_position(self, source, job_type, position) -> None: ...
    def reset_source_position(self, source, job_type) -> None: ...
    def enqueue_pending_reply(self, username, **fields) -> None: ...
    def update_pending_reply(self, username, **fields) -> None: ...
    def get_pending_replies(self): ...
    def remove_pending_reply(self, username) -> None: ...
